...
import os
os.environ["OGR_GEOJSON_MAX_OBJ_SIZE"] = "0"  # 0 表示取消限制（单位：MB）
...
class Processor:
    ...
    def __init__(
        ...
        clinical_csv: Optional[str] = None,  # 新增：接收临床数据路径
    ) -> None:
        """
        ...
        # === Initialize WSIs ===
            # 加载临床数据（新增）
        self.clinical_map = None
        if clinical_csv is not None:  # 需要从args传入clinical_csv
            import pandas as pd
            self.clinical_df = pd.read_csv(clinical_csv)
        # 确保slide_id是字符串类型（与文件夹名称匹配）
            self.clinical_df['slide_id'] = self.clinical_df['slide_id'].astype(str)
        # 建立slide_id到临床特征的映射
            self.clinical_map = self.clinical_df.set_index('slide_id').to_dict('index')
        self.wsis = []
        for wsi_idx, abs_path in enumerate(full_paths):
            rel_path = rel_paths[wsi_idx]  
            name = rel_path.replace(os.sep, '_')
            name = re.sub(r'[^\w]', '_', rel_path.replace(os.sep, '_'))  # 替换非字母数字字符为下划线  
        ...
        # 提取slide_id（根据文件夹名称，假设路径格式为 .../slide_id/xxx.wsi）
            slide_dir = os.path.dirname(abs_path)
            slide_id = os.path.basename(slide_dir)  # 文件夹名称即为slide_id

        # 获取当前WSI对应的临床数据
            clinical_data = self.clinical_map.get(slide_id, None) if self.clinical_map else None
            slide = load_wsi(
                slide_path=abs_path,
                name=name,
                ...
                clinical_data=clinical_data  # 新增：传递临床数据
            )
    ...
    def run_segmentation_job(
        ...
        for wsi in self.loop:   
            ...
            try:
                ...
                # 替换原来的pyogrio.set_gdal_config_option行
                gdal.SetConfigOption("OGR_GEOJSON_MAX_OBJ_SIZE", "0")  # 允许任意大小的对象
    ...
    def run_slide_feature_extraction_job(
        ...
        # Setting I/O
        slide_to_patch_encoder_name = {'abmil': 'resnet50',}
