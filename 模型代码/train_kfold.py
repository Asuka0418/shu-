...
# 3) 训练 & 评估
# --------------------------
# def train_one_epoch(model, loader, opt, device, num_classes, cls_w: torch.Tensor = None, lambda_att: float = 0.0):
#     model.train()
#     tot = 0.0
#     for batch in loader:
#         feat = batch['feat'][0].to(device)
#         xy = batch['xy'][0].to(device)
#         clin = batch['clin'][0].to(device)
#         y    = batch['label'].to(device).view(-1).long()  # [1]

#         logits, aux = model(feat, xy, clin)
#         if num_classes >= 2:
#             loss = F.cross_entropy(logits.unsqueeze(0), y, weight=cls_w)
#         else:
#             loss = F.binary_cross_entropy_with_logits(logits.view(()), y.float())
