...
from trident.utils import encode_clinical_features  # 导入临床特征编码工具
import torch
...
class ABMILSlideEncoder(BaseSlideEncoder):
    ...
    def __init__(self, **build_kwargs):
        ...
        # 初始化临床特征处理层（在_build中动态定义维度）
        self.clinical_encoder = None  # 临床特征维度映射层
        self.fusion_layer = None      # 特征融合层

    def _build(
        self,
        input_feature_dim: int,  # patch特征维度（如2048）
        n_heads: int,
        head_dim: int,
        dropout: float,
        gated: bool,
        ...
        # 初始化临床特征处理层（根据CSV特征编码后维度=26）
        CLINICAL_FEATURE_DIM = 26  # 与utils.encode_clinical_features输出维度一致
        self.clinical_encoder = nn.Sequential(
            nn.Linear(CLINICAL_FEATURE_DIM, input_feature_dim),  # 映射到与图像特征同维度
            nn.GELU(),
            nn.Dropout(dropout)
        )
        ...
        self.fusion_layer = nn.Sequential(
            nn.Linear(2 * input_feature_dim, input_feature_dim),  # 2D -> D
            nn.GELU()
        )
        
        precision = torch.float32
        embedding_dim = input_feature_dim  # 融合后仍保持原图像特征维度
        return model, precision, embedding_dim

    def forward(self, batch, device='cuda', return_raw_attention=False):
            # ====== 新增：检查并调整特征维度 ======
        device = next(self.parameters()).device  # 获取模型所在设备（保持与原代码一致）
        features = batch['features'].to(device)  # 将特征移至设备
        
        # 打印当前特征形状（用于调试，可保留或删除）
        print(f"ABMIL输入特征原始形状：{features.shape}")
        
        # 调整维度至 [batch_size=1, num_instances, feature_dim]
        if features.dim() == 2:
            # 若输入是二维 (num_instances, feature_dim)，增加batch维度
            features = features.unsqueeze(0)  # 变为 (1, num_instances, feature_dim)
            print(f"调整后特征形状：{features.shape}")
        elif features.dim() == 3 and features.shape[0] != 1:
            # 若batch维度不为1，强制设为1（单slide处理场景）
            features = features[:1]  # 取第一个样本作为batch
            print(f"调整后特征形状：{features.shape}")
        
        # ====== 原有代码修改：使用调整后的features ======
        image_features = self.model['pre_attention_layers'](features)  # 第203行，修改为使用调整后的features
        # 1. 处理图像特征（原始ABMIL逻辑）
        image_features = self.model['pre_attention_layers'](batch['features'].to(device))
        image_features, attn = self.model['image_pooler'](image_features)  # (b, 1, f)
        image_features = rearrange(image_features, 'b 1 f -> b f')  # (b, f)
        image_features = self.model['post_attention_layers'](image_features)  # (b, f)
        
        # 2. 处理临床特征并融合
        clinical_data = batch.get('clinical_data')  # 从batch中获取临床数据（dict列表）
        if clinical_data is not None and self.clinical_encoder is not None:
            # 2.1 编码临床特征（list of dict -> tensor）
            # 注意：batch['clinical_data']应为列表，每个元素是一个样本的临床特征dict
            clinical_tensors = [encode_clinical_features(data) for data in clinical_data]
            clinical_features = torch.stack(clinical_tensors).to(device)  # (b, 26)
            
            # 2.2 将临床特征映射到与图像特征同维度
            clinical_features = self.clinical_encoder(clinical_features)  # (b, f)
            
            # 2.3 融合图像特征和临床特征（拼接后通过融合层）
            fused_features = torch.cat([image_features, clinical_features], dim=1)  # (b, 2f)
            image_features = self.fusion_layer(fused_features)  # (b, f)
